<!-- ===================== HERO BANNER ===================== -->
<!-- Generated from profile.json by .github/scripts/generate_banner.py -->
<!--
  Six renders, one per breakpoint x theme. <picture> media queries are the only
  responsive mechanism GitHub's markdown pipeline honours — class attributes are
  stripped and there is no stylesheet to hook into — so the breakpoints live in
  the media queries and the layout switch happens inside the SVG itself.

      >= 1024px (lg, xl)  dark.svg      / light.svg      two columns
      640-1023px (md)     dark-md.svg   / light-md.svg   stacked
      <= 639px  (sm)      dark-sm.svg   / light-sm.svg   stacked, condensed

  First matching <source> wins, so the narrow queries must come first.
-->

<picture>
  <source media="(max-width: 639px) and (prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/dark-sm.svg">
  <source media="(max-width: 639px)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/light-sm.svg">
  <source media="(max-width: 1023px) and (prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/dark-md.svg">
  <source media="(max-width: 1023px)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/light-md.svg">
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/dark.svg">
  <img width="100%" alt="Owais Hassan Wani — Full Stack Developer" src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/light.svg">
</picture>

<!-- ===================== TYPING HEADLINE ===================== -->
<!-- Canvas width and type size step down with the viewport so the headline
     stays legible instead of being scaled to a few pixels tall on a phone -->

<div align="center">

<picture>
  <source media="(max-width: 639px)" srcset="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=15&pause=1000&color=3B82F6&center=true&vCenter=true&width=340&height=40&lines=Full+Stack+Developer;React+%7C+Next.js+%7C+TypeScript;Node.js+%7C+Express+%7C+PostgreSQL;Building+scalable%2C+user-centric+web+apps">
  <source media="(max-width: 1023px)" srcset="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=20&pause=1000&color=3B82F6&center=true&vCenter=true&width=560&height=48&lines=Full+Stack+Developer;React+%7C+Next.js+%7C+TypeScript;Node.js+%7C+Express+%7C+PostgreSQL;Building+scalable%2C+user-centric+web+apps">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=24&pause=1000&color=3B82F6&center=true&vCenter=true&width=760&lines=Full+Stack+Developer;React+%7C+Next.js+%7C+TypeScript;Node.js+%7C+Express+%7C+PostgreSQL;Building+scalable%2C+user-centric+web+apps" alt="Full Stack Developer — React, Next.js, TypeScript, Node.js, PostgreSQL" />
</picture>

</div>

<!-- ===================== BADGES ===================== -->

<!--
  Badges are separated by plain whitespace rather than &nbsp; so a narrow
  column can break the row between them instead of overflowing.

  shields.io no longer ships a `linkedin` icon (simple-icons dropped the mark),
  so `logo=linkedin` silently renders a badge with no glyph. The LinkedIn
  badge below carries the mark as an inline data-URI instead; the fill is baked
  into the SVG because shields does not apply logoColor to custom logos.
-->

<div align="center">

<a href="https://www.linkedin.com/in/owaishassanwani/">
  <img src="https://img.shields.io/badge/LinkedIn-Connect-0D1117?style=for-the-badge&labelColor=0D1117&logo=data:image/svg%2Bxml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAyNCAyNCIgZmlsbD0iIzNCODJGNiI%%2BPC9zdmc%2B" alt="LinkedIn" />
</a>
<a href="https://wani-owais.github.io/My-Portfolio/">
  <img src="https://img.shields.io/badge/Portfolio-Visit-0D1117?style=for-the-badge&logo=googlechrome&logoColor=06B6D4&labelColor=0D1117" alt="Portfolio" />
</a>
<a href="mailto:owaishassanwani@gmail.com">
  <img src="https://img.shields.io/badge/Email-Reach%20out-0D1117?style=for-the-badge&logo=gmail&logoColor=8B5CF6&labelColor=0D1117" alt="Email" />
</a>

<br/><br/>

<a href="https://github.com/WANI-OWAIS?tab=followers">
  <img src="https://img.shields.io/github/followers/WANI-OWAIS?style=for-the-badge&logo=github&logoColor=F8FAFC&label=Followers&labelColor=0D1117&color=3B82F6" alt="GitHub followers" />
</a>
<a href="https://github.com/WANI-OWAIS?tab=repositories&sort=stargazers">
  <img src="https://img.shields.io/github/stars/WANI-OWAIS?style=for-the-badge&logo=github&logoColor=F8FAFC&label=Stars&labelColor=0D1117&color=06B6D4" alt="Total stars" />
</a>
<img src="https://komarev.com/ghpvc/?username=WANI-OWAIS&style=for-the-badge&label=Profile+Views&color=8B5CF6" alt="Profile views" />

</div>

<br/>

<!-- ===================== ABOUT ===================== -->

## <img src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/.github/assets/spark.svg" width="18" /> About Me

I'm **Owais Hassan Wani**, a Full Stack Developer from **Jammu & Kashmir, India**, currently pursuing a **B.Tech in Computer Science & Engineering at NMAM Institute of Technology (NMAMIT)**.

I build end-to-end web applications — from schema design and API architecture through to the interface people actually touch. Most of my time goes into the React and Next.js ecosystem, backed by Node.js services and relational data models.

```ts
const owais = {
  role:     "Full Stack Developer",
  focus:    ["React", "Next.js", "TypeScript", "Node.js"],
  building: "Scalable products with interfaces worth using",
  believes: "Clean architecture outlives clever code",
  learning: ["System Design", "Advanced TypeScript", "Cloud Infrastructure"],
};
```

**What I care about**

- Designing systems that stay maintainable as they grow
- REST API design and backend architecture that ages well
- Interfaces that feel considered, accessible and fast
- Contributing to open source and learning in the open

<br/>

<!-- ===================== TECH STACK ===================== -->

## <img src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/.github/assets/spark.svg" width="18" /> Tech Stack

<table>
<tr>
<td valign="top" width="50%">

**Languages**

![JavaScript](https://img.shields.io/badge/JavaScript-0D1117?style=flat-square&logo=javascript&logoColor=F7DF1E)
![TypeScript](https://img.shields.io/badge/TypeScript-0D1117?style=flat-square&logo=typescript&logoColor=3178C6)
![C](https://img.shields.io/badge/C-0D1117?style=flat-square&logo=c&logoColor=A8B9CC)
![C++](https://img.shields.io/badge/C++-0D1117?style=flat-square&logo=cplusplus&logoColor=00599C)

**Frontend**

![React](https://img.shields.io/badge/React-0D1117?style=flat-square&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-0D1117?style=flat-square&logo=nextdotjs&logoColor=F8FAFC)
![HTML5](https://img.shields.io/badge/HTML5-0D1117?style=flat-square&logo=html5&logoColor=E34F26)
![CSS3](https://img.shields.io/badge/CSS3-0D1117?style=flat-square&logo=css3&logoColor=1572B6)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-0D1117?style=flat-square&logo=tailwindcss&logoColor=06B6D4)

**Backend**

![Node.js](https://img.shields.io/badge/Node.js-0D1117?style=flat-square&logo=nodedotjs&logoColor=339933)
![Express](https://img.shields.io/badge/Express-0D1117?style=flat-square&logo=express&logoColor=F8FAFC)
![REST API](https://img.shields.io/badge/REST%20APIs-0D1117?style=flat-square&logo=fastapi&logoColor=06B6D4)
![Socket.IO](https://img.shields.io/badge/Socket.IO-0D1117?style=flat-square&logo=socketdotio&logoColor=F8FAFC)

</td>
<td valign="top" width="50%">

**Database**

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-0D1117?style=flat-square&logo=postgresql&logoColor=4169E1)
![MongoDB](https://img.shields.io/badge/MongoDB-0D1117?style=flat-square&logo=mongodb&logoColor=47A248)
![Prisma](https://img.shields.io/badge/Prisma-0D1117?style=flat-square&logo=prisma&logoColor=2D3748)
![Neon](https://img.shields.io/badge/Neon-0D1117?style=flat-square&logo=postgresql&logoColor=00E599)

**Authentication**

![Auth.js](https://img.shields.io/badge/Auth.js-0D1117?style=flat-square&logo=auth0&logoColor=8B5CF6)
![JWT](https://img.shields.io/badge/JWT-0D1117?style=flat-square&logo=jsonwebtokens&logoColor=F8FAFC)

**Tools & Platforms**

![Git](https://img.shields.io/badge/Git-0D1117?style=flat-square&logo=git&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-0D1117?style=flat-square&logo=github&logoColor=F8FAFC)
![VS Code](https://img.shields.io/badge/VS%20Code-0D1117?style=flat-square&logo=visualstudiocode&logoColor=007ACC)
![Vercel](https://img.shields.io/badge/Vercel-0D1117?style=flat-square&logo=vercel&logoColor=F8FAFC)
![Postman](https://img.shields.io/badge/Postman-0D1117?style=flat-square&logo=postman&logoColor=FF6C37)

**Integrations**

![Razorpay](https://img.shields.io/badge/Razorpay-0D1117?style=flat-square&logo=razorpay&logoColor=3395FF)
![Nodemailer](https://img.shields.io/badge/Nodemailer-0D1117?style=flat-square&logo=minutemailer&logoColor=06B6D4)

</td>
</tr>
</table>

<br/>

<!-- ===================== EXPERIENCE ===================== -->

## <img src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/.github/assets/spark.svg" width="18" /> Experience

<table>
<tr>
<td width="4"></td>
<td>

### Full Stack Developer Intern
**Vertex Kashmir Holidays** &nbsp;·&nbsp; Jammu & Kashmir, India

Building and maintaining a production travel booking platform serving real customers.

- Developing full-stack features across a **Next.js**, **TypeScript** and **Tailwind CSS** frontend
- Modelling and querying relational data with **Prisma ORM** on **Neon PostgreSQL**
- Implementing secure session handling and protected routes with **Auth.js**
- Shipping to production on **Vercel** with an iterative release cycle

</td>
</tr>
<tr>
<td width="4"></td>
<td>

### B.Tech, Computer Science & Engineering
**NMAM Institute of Technology (NMAMIT)**

Core coursework in data structures, algorithms, databases, operating systems and computer networks, applied through self-directed full-stack project work.

</td>
</tr>
</table>

<br/>

<!-- ===================== FEATURED PROJECTS ===================== -->

## <img src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/.github/assets/spark.svg" width="18" /> Featured Projects

<!-- Panel is generated from projects.json and published to the `projects` branch -->

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/projects/projects.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/projects/projects-light.svg" />
  <img width="100%" src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/projects/projects-light.svg" alt="Featured projects" />
</picture>

</div>

<!--
  One project per row rather than three across. A three-column table has a
  min-content width of ~385px, which no phone column can satisfy, so GitHub's
  own overflow-x on tables turned it into a sideways swipe. Stacking mirrors
  the Experience section's single-column row pattern and reflows at any width.
-->

<table>
<tr>
<td width="4"></td>
<td valign="top">

**Vertex Kashmir Holidays**

Production travel booking platform with curated packages, itinerary management and authenticated bookings.

`Next.js` `React` `TypeScript` `Tailwind CSS` `Prisma` `Neon PostgreSQL` `Auth.js`

[**Visit the live site →**](https://vertexkashmirholidays.com)

</td>
</tr>
<tr>
<td width="4"></td>
<td valign="top">

**Real-Time Collaborative Code Editor**

Multi-user editor where several people edit the same document at once, with live cursor synchronisation, shared rooms and in-browser code execution.

`React` `Node.js` `Socket.IO`

[**View repository →**](https://github.com/WANI-OWAIS/RealTime-CodeEditor-Collaborative)

</td>
</tr>
<tr>
<td width="4"></td>
<td valign="top">

**StudyNotion**

Full-stack EdTech platform covering course authoring, a media upload pipeline and integrated payment checkout.

`React` `Node.js` `Express` `MongoDB` `Cloudinary` `Razorpay`

[**View repository →**](https://github.com/WANI-OWAIS/StudyNotion-EdTech)

</td>
</tr>
</table>

<div align="center">

[![All repositories](https://img.shields.io/badge/Browse%20all%20repositories-0D1117?style=for-the-badge&logo=github&logoColor=3B82F6&labelColor=0D1117)](https://github.com/WANI-OWAIS?tab=repositories)

</div>

<br/>

<!-- ===================== GITHUB STATS ===================== -->

## <img src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/.github/assets/spark.svg" width="18" /> GitHub Stats & Top Languages

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://streak-stats.demolab.com/?user=WANI-OWAIS&hide_border=true&background=0D1117&stroke=06B6D4&ring=8B5CF6&fire=3B82F6&currStreakLabel=06B6D4&sideLabels=94A3B8&currStreakNum=F8FAFC&sideNums=F8FAFC&dates=475569&titleColor=3B82F6&card_width=1180" />
  <img width="100%" src="https://streak-stats.demolab.com/?user=WANI-OWAIS&hide_border=true&background=FFFFFF&stroke=0891B2&ring=7C3AED&fire=2563EB&currStreakLabel=0891B2&sideLabels=475569&currStreakNum=0F172A&sideNums=0F172A&dates=94A3B8&titleColor=2563EB&card_width=1180" alt="Contribution streak" />
</picture>

<br/><br/>

<!-- Metrics + top languages are rendered from the GitHub API by
     .github/scripts/generate_stats.py and published to the `stats` branch -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/stats/stats.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/stats/stats-light.svg" />
  <img width="100%" src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/stats/stats-light.svg" alt="GitHub metrics and most-used languages" />
</picture>

</div>

<br/>

<!-- ===================== CONTRIBUTION SNAKE ===================== -->

## <img src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/.github/assets/spark.svg" width="18" /> Contribution Graph

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/output/snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/output/snake-light.svg" />
  <img width="100%" src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/output/snake-light.svg" alt="Contribution snake animation" />
</picture>

</div>

<br/>

<!-- ===================== TROPHIES ===================== -->

## <img src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/.github/assets/spark.svg" width="18" /> GitHub Trophies

<div align="center">

<!-- Tiered achievements rendered from live API data, published to the `stats` branch -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/stats/trophies.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/stats/trophies-light.svg" />
  <img width="100%" src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/stats/trophies-light.svg" alt="GitHub achievement trophies" />
</picture>

</div>

<br/>

<!-- ===================== CURRENT FOCUS ===================== -->

## <img src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/.github/assets/spark.svg" width="18" /> Current Focus

<!-- Stacked for the same reason as Featured Projects: two columns of bulleted
     prose cannot reflow below ~300px, so the pairs run vertically instead. -->

<table>
<tr>
<td width="4"></td>
<td valign="top">

**Building**

- Production features for the Vertex Kashmir Holidays platform
- Full-stack side projects in the Next.js and TypeScript ecosystem

**Deepening**

- Advanced TypeScript patterns and type-level design
- System design and scalable backend architecture

</td>
</tr>
<tr>
<td width="4"></td>
<td valign="top">

**Exploring**

- Cloud infrastructure and deployment pipelines
- Real-time systems built on WebSockets
- Database performance and query optimisation

**Open to**

- Open-source collaboration
- Full-stack development opportunities

</td>
</tr>
</table>

<br/>

<!-- ===================== FOOTER ===================== -->

<div align="center">

<img width="100%" src="https://raw.githubusercontent.com/WANI-OWAIS/WANI-OWAIS/main/.github/assets/footer.svg" alt="" />

<sub>Designed and built by <b>Owais Hassan Wani</b> · Banner and project panels generated from <code>profile.json</code> and <code>projects.json</code></sub>

</div>
